...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <Link href={`/productos/${category}/${slug}`} className="group block">
      ...
      className="group relative rounded-[2.5rem] md:rounded-[3.5rem] flex flex-col h-full cursor-pointer overflow-hidden"
    >
      {/* Image Container with rounding fix */}
      <div className="relative aspect-[3/4.5] overflow-hidden rounded-[2rem] md:rounded-[2.5rem] bg-white/5 border border-white/10 group-hover:border-primary/20 transition-all duration-500">
        <motion.img
        ...
        <div className="absolute inset-0 pointer-events-none rounded-[2rem] md:rounded-[2.5rem] ring-1 ring-inset ring-white/10" />
      </div>
      </motion.div>
    </Link>
  );
