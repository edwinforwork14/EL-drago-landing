...
<motion.div
          ...
          className="relative w-full md:w-1/2 max-h-[80vh] rounded-[3rem] overflow-hidden shadow-2xl group border border-primary/5"
        >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 md:p-12 z-20">
              <div className="flex flex-wrap items-center gap-4 mb-4">
                <span className="bg-accent text-primary-dark text-[9px] font-bold uppercase tracking-widest px-3.5 py-1.5 rounded-full shadow-lg">{recipe.category}</span>
                <span className="text-white/80 text-xs font-bold uppercase tracking-wider flex items-center gap-1.5"><span className="material-symbols-outlined text-base text-accent">schedule</span>{recipe.time}</span>
                <span className="text-white/80 text-xs font-bold uppercase tracking-wider flex items-center gap-1.5"><span className="material-symbols-outlined text-base text-accent">signal_cellular_alt</span>{recipe.difficulty}</span>
              </div>
              <h1 className="text-white font-[family-name:var(--font-luckiest-guy)] text-3xl md:text-5xl lg:text-6xl uppercase leading-none tracking-wide drop-shadow-md">{recipe.title}</h1>
            </div>
          </motion.div>
          ...
          <div className="w-full md:w-1/2 space-y-12">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2, duration: 0.7 }}>
              <div className="bg-[#FCF2E6]/60 backdrop-blur-md rounded-[2.5rem] p-8 border border-primary/5 shadow-xl sticky top-28">
                <span className="text-primary font-bold text-[9px] uppercase tracking-[0.25em] mb-2 block">Paso a Paso</span>
                <h2 className="text-primary-dark font-[family-name:var(--font-luckiest-guy)] text-3xl uppercase tracking-tight mb-6">Ingredientes</h2>
                <ul className="space-y-4">
                  {recipe.ingredients.map((ing, i) => {
                    const checked = checkedIngredients[i] || false;
                    return (
                      <li key={i} onClick={() => toggleIngredient(i)} className="flex items-center gap-3.5 text-primary-dark/80 font-medium cursor-pointer select-none group/item py-1">
                        <span className="material-symbols-outlined text-2xl transition-colors duration-300 shrink-0 text-primary">{checked ? "check_circle" : "radio_button_unchecked"}</span>
                        <span className={`text-base leading-relaxed transition-all duration-300 ${checked ? "line-through opacity-40 text-primary-dark" : "group-hover/item:text-primary"}`}>{ing}</span>
                      </li>
                    );
                  })}
                </ul>
...
<div className="bg-[#FCF2E6]/60 backdrop-blur-md rounded-[2rem] p-6 border border-primary/5 shadow-lg">
                  <span className="text-primary font-bold text-[9px] uppercase tracking-[0.25em] mb-2 block">
                    En la Cocina
                  </span>
                  <h2 className="text-primary-dark font-[family-name:var(--font-luckiest-guy)] text-2xl uppercase mb-4">
                    Preparación
                  </h2>
                  <div className="space-y-4">
                    {recipe.preparation.map((step, i) => (
                      <div key={i} className="flex gap-4 relative group/step">
                        <span className="shrink-0 w-8 h-8 md:w-10 md:h-10 bg-primary text-white border border-primary/15 rounded-full flex items-center justify-center font-bold text-sm md:text-base shadow-md z-10 transition-transform duration-300 group-hover/step:scale-110">
                          {i + 1}
                        </span>
                        <p className="text-primary-dark/70 text-sm md:text-base leading-snug font-medium">
                          {step}
                        </p>
                      </div>
                    ))}
                ...
                <div className="mt-16 pt-10 border-t border-primary/10">
                  <h3 className="text-primary-dark font-[family-name:var(--font-luckiest-guy)] text-lg uppercase tracking-wide mb-3">¿Preparaste esta deliciosa receta?</h3>
                  <p className="text-primary-dark/60 text-sm font-medium mb-6 leading-relaxed">¡Compártenos tu experiencia! Escríbenos para contarnos qué tal te quedó o consultarnos cualquier duda culinaria.</p>
                </div>
              </div>
            </motion.div>
          </div>
        ...
        <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5, duration: 0.7 }} className="mt-32 border-t border-primary/10 pt-20">
          <span className="text-primary font-bold text-xs uppercase tracking-[0.25em] mb-4 block text-center">Inspiración Culinaria</span>
          <h2 className="text-primary-dark font-[family-name:var(--font-luckiest-guy)] text-3xl md:text-5xl uppercase tracking-tight text-center mb-12">Descubre <span className="text-primary font-[family-name:var(--font-mr-dafoe)] normal-case text-[3.5rem] md:text-[5rem] -rotate-1 inline-block">Más Recetas</span></h2>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {recipes.filter((r) => r.id !== recipe.id).slice(0, 3).map((r, i) => (
              <motion.div key={r.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 + 0.05 * i }}>
                <Link href={`/receta/${r.slug}`} className="group block bg-[#FCF2E6]/60 backdrop-blur-md rounded-[2.5rem] p-5 border border-primary/5 hover:border-primary/20 transition-all duration-500 hover:shadow-lg hover:-translate-y-1">
                  ...
                  <div className="px-2">
                    <h3 className="text-primary-dark font-[family-name:var(--font-luckiest-guy)] text-lg uppercase tracking-tight group-hover:text-primary transition-colors leading-tight mb-2">{r.title}</h3>
                    <div className="flex items-center gap-3 text-primary-dark/45 text-[10px] font-bold uppercase tracking-wider"><span className="text-accent">{r.category}</span><span>•</span><span>{r.time}</span></div>
                  </div>
                </Link>
              </motion.div>
            ))}
