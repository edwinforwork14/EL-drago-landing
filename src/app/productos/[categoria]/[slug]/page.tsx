    ...
    <div className="min-h-screen bg-background flex flex-col justify-between overflow-x-hidden">
      ...
      <main className="pt-32 px-6 md:px-12 max-w-7xl mx-auto pb-24 flex-1 w-full">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-center">
          ...
          >
            ...
            <div className="relative aspect-[4/5] w-full max-w-[480px] rounded-[3rem] overflow-hidden bg-white/40 backdrop-blur-md border border-primary/5 shadow-2xl group">
              <img src={product.image} alt={product.name} className="w-full h-full object-cover transition-transform duration-[1s] group-hover:scale-105" />
              {product.tag && (
          ...
          >
            <span className="text-primary font-bold text-xs uppercase tracking-[0.25em] mb-4 block">{product.category}</span>
            ...
            <p className="text-primary-dark/70 text-base md:text-lg leading-relaxed font-medium mb-8">{product.description}</p>
            ...
            <div className="bg-[#FCF2E6]/50 backdrop-blur-md rounded-[2rem] p-6 border border-primary/5 mb-10 flex gap-5 items-start">
              ...
              <div>
                <h4 className="text-primary-dark font-[family-name:var(--font-luckiest-guy)] uppercase text-lg mb-2 tracking-wide">{pairing.title}</h4>
                <p className="text-primary-dark/65 text-sm leading-relaxed font-medium">{pairing.desc}</p>
              </div>
            </div>
        ...
        {siblings.length > 0 && (
          ...
          >
            <span className="text-primary font-bold text-xs uppercase tracking-[0.25em] mb-4 block text-center">Recomendación Gourmet</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {siblings.map((sibling, i) => {
                ...
                const siblingCategory = getCategorySlugForProduct(sibling) || 'productos';
                return (
                  <motion.div key={sibling.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 + 0.05 * i }}>
                    <Link
                      href={`/productos/${siblingCategory}/${siblingSlug}`}
                    ...
                    >
                      ...
                      <div className="px-2">
                        ...
                        <span className="text-primary-dark/45 text-[10px] font-bold uppercase tracking-wider block">{sibling.category}</span>
                      </div>
                    </Link>
                  </motion.div>
                );
